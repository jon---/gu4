...
public class enemy230Controller : MonoBehaviour {
	//public
	...
	const float yspdbase = 0.05f;
	const float spdbase = 0.05f;
	//base hit point
	...
	//shot
	const float bspd = 0.52f;
	const float bdir1 = 10.0f;
	const float bdir2 = 5.0f;
	...
	void Update () {
		...
		if (intervalCnt >= 1) {
			...
			case 0:
				...
				if ( bcnt == intv ) {
					//shot bullet
					const float b1spd = 0.20f;
					if (mc.gameLevel == mc.gameLevelHard) {
						mc.generateEnemyBullet120 (ddir - 46.0f, bspd + b1spd - 0.10f, cashTransform.position.x, cashTransform.position.y);
						mc.generateEnemyBullet120 (ddir - 42.0f, bspd + b1spd - 0.10f, cashTransform.position.x, cashTransform.position.y);
					}
					if (mc.gameLevel != mc.gameLevelEasy) {
						mc.generateEnemyBullet120 (ddir - 30.0f, bspd + b1spd - 0.05f, cashTransform.position.x, cashTransform.position.y);
						mc.generateEnemyBullet120 (ddir - 24.0f, bspd + b1spd - 0.05f, cashTransform.position.x, cashTransform.position.y);
						mc.generateEnemyBullet120 (ddir - 12.0f, bspd + b1spd, cashTransform.position.x, cashTransform.position.y);
					}
					mc.generateEnemyBullet120 (ddir-6.0f, bspd+b1spd, cashTransform.position.x, cashTransform.position.y);
					mc.generateEnemyBullet120 (ddir+6.0f, bspd+b1spd, cashTransform.position.x, cashTransform.position.y);
					if (mc.gameLevel != mc.gameLevelEasy) {
						mc.generateEnemyBullet120 (ddir + 12.0f, bspd + b1spd, cashTransform.position.x, cashTransform.position.y);
						mc.generateEnemyBullet120 (ddir + 24.0f, bspd + b1spd - 0.05f, cashTransform.position.x, cashTransform.position.y);
			...
			case 1:
				...
				if ((tdir > cdir) && ((tdir - cdir) > dspd2)) {
					if ((tdir - cdir) < 180) {
						cdir = cdir + dspd2;
					} else {
						cdir = cdir - dspd2;
					}
				} else if ((tdir < cdir) && ((cdir - tdir) > dspd2)) {
					if ((cdir - tdir) < 180) {
						cdir = cdir - dspd2;
					} else {
						cdir = cdir + dspd2;
				...
				const float dstns = 4.4f;//3.4f
				if ((Mathf.Abs (xdistance) <= dstns) && (Mathf.Abs (ydistance) <= dstns) &&
					(Mathf.Abs (xdistance) >= dstns2) && (Mathf.Abs (ydistance) >= dstns2) &&
					(Mathf.Abs (tdir - cdir) <= 45.0f) &&
					((cashTransform.position.y <= 4.5f) && (cashTransform.position.y >= -4.5f)) &&
			...
			case 2:
				...
				if (bcnt % intv == 0) {
					//shot bullet
					for (int i = 0; i < 360; i = i + dd) {
						mc.generateEnemyBullet120 (cdir + i, bspd, cashTransform.position.x, cashTransform.position.y);
					}
				}
				bcnt++;
				if (bcnt > intvmax) {
