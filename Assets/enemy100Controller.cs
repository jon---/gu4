...
public class enemy100Controller : MonoBehaviour {
	//public
		...
		if (intervalCnt >= 1) {
			...
			case 1:
				...
				if(bcnt % intv == 0){
					...
					float bx;
					float by;
					const float bspd = 0.8f;
					bxc = Mathf.Cos (cdir * Mathf.Deg2Rad) * 1.0f;
					byc = Mathf.Sin (cdir * Mathf.Deg2Rad) * 1.0f;
					mc.generateEnemyBullet100 (1, cashTransform.position.x, cashTransform.position.y, bxc*0.28f, byc*0.28f, bxc*bspd, byc*bspd);
					if ( (mc.gameLevel == mc.gameLevelNormal) || (mc.gameLevel == mc.gameLevelHard) ) {
						bx = Mathf.Cos ((cdir + 2.2f) * Mathf.Deg2Rad) * 1.0f;
						by = Mathf.Sin ((cdir + 2.2f) * Mathf.Deg2Rad) * 1.0f;
						mc.generateEnemyBullet100 (1, cashTransform.position.x, cashTransform.position.y, bxc * 0.28f, byc * 0.28f, bx * bspd, by * bspd);
						bx = Mathf.Cos ((cdir - 2.2f) * Mathf.Deg2Rad) * 1.0f;
						by = Mathf.Sin ((cdir - 2.2f) * Mathf.Deg2Rad) * 1.0f;
						mc.generateEnemyBullet100 (1, cashTransform.position.x, cashTransform.position.y, bxc * 0.28f, byc * 0.28f, bx * bspd, by * bspd);
					}
					if ( mc.gameLevel == mc.gameLevelHard ) {
						bx = Mathf.Cos ((cdir + 4.4f) * Mathf.Deg2Rad) * 1.0f;
						by = Mathf.Sin ((cdir + 4.4f) * Mathf.Deg2Rad) * 1.0f;
						mc.generateEnemyBullet100 (1, cashTransform.position.x, cashTransform.position.y, bxc * 0.28f, byc * 0.28f, bx * bspd, by * bspd);
						bx = Mathf.Cos ((cdir - 4.4f) * Mathf.Deg2Rad) * 1.0f;
						by = Mathf.Sin ((cdir - 4.4f) * Mathf.Deg2Rad) * 1.0f;
						mc.generateEnemyBullet100 (1, cashTransform.position.x, cashTransform.position.y, bxc * 0.28f, byc * 0.28f, bx * bspd, by * bspd);
