...
public class playerBullet110Controller : MonoBehaviour {
	//public
	...
	//x,y speed base
	const float xspd_base = 1.0f;//0.6f;//0.8f;
	const float yspd_base = 1.0f;//0.6f;//0.8f;
	...
	void Update () {
		...
		if ( (cashTransform.position.x >= xmax ) || (cashTransform.position.x <= xmin) || 
			...
			if (incobj == true) {
				...
				#if UNITY_EDITOR
				Debug.Log ("no inc dec playerbullet110");
				#endif
			}
			//delete this
			alreadydelete = true;
