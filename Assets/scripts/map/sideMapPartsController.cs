...
public class sideMapPartsController : MonoBehaviour {
	...
	//const
	//side map draw
	const float xoffset = -3.5f;	//side map draw offset x
	const float yoffset = -6.0f;	//side map draw offset y
	const int map_xsize = 8;	//side map draw size x
	const int map_ysize = 12;	//side map draw size y
	const float sm_spdy = 1.5f;	//side map y scroll speed base
	const float sm_spdx = 1.5f;	//side map x scroll speed base
...
//		mc.incObj();
	...
	void Update () {
		...
		if (intervalCnt >= 1) {
			...
			speed_y = mpc.getScrSpd() * sm_spdy;

			//scroll x move
			mpxmov = mpc.getMapxMov () * sm_spdx;
			...
			if ( (cashTransform.position.y <= yoffset) && (speed_y>=0)) {
				...
				sr.sprite = mpc.getSideMapNextData (xindex);
